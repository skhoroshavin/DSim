...
{
    dsim_array_resize( &h->_hash, count );
...
{
    dsim_array_init( &h->keys, alloc );
    dsim_array_init( &h->_hash, alloc );
    dsim_array_init( &h->_next, alloc );
}
...
void dsim_hash_reserve( struct dsim_hash *h, uint32_t count )
{
    ...
    dsim_array_reserve( &h->keys, count );
    dsim_array_reserve( &h->_next, count );
...
void dsim_hash_push_back( struct dsim_hash *h, uint64_t key )
{
    ...
    dsim_array_push_back( &h->keys, key );
    dsim_array_push_back( &h->_next, DSIM_INVALID_INDEX );
...
void dsim_hash_push_back_n( struct dsim_hash *h, const uint64_t *keys, uint32_t count )
{
    ...
    dsim_array_push_back_n( &h->keys, keys, count );
    dsim_array_resize( &h->_next, start_pos + count );
...
void dsim_hash_remove_fast( struct dsim_hash *h, uint32_t pos, uint32_t count )
{
    ...
    dsim_array_pop_back_n( &h->keys, count );
    dsim_array_pop_back_n( &h->_next, count );
}

void dsim_hash_clear( struct dsim_hash *h )
{
    dsim_array_clear( &h->keys );
    dsim_array_clear( &h->_next );
    dsim_fill( h->_hash.data, DSIM_INVALID_INDEX, h->_hash.count );
}

void dsim_hash_reset( struct dsim_hash *h )
{
    dsim_array_reset( &h->keys );
    dsim_array_reset( &h->_hash );
    dsim_array_reset( &h->_next );
}
