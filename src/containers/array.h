...
#define DSIM_DEFINE_ARRAY(type) \
    ...
    inline static void dsim_##type##_array##_reserve( dsim_##type##_array *array, uint32_t count ) \
    { _dsim_array_reserve( (_dsim_array*)array, count, sizeof(type)); } \
    inline static void dsim_##type##_array_resize( dsim_##type##_array *array, uint32_t count ) \
    { _dsim_array_resize( (_dsim_array*)array, count, sizeof(type) ); } \
    inline static void dsim_##type##_array_fill( dsim_##type##_array *array, type value ) \
    { _dsim_array_fill( (_dsim_array*)array, &value, sizeof(type) ); } \
    inline static void dsim_##type##_array_push_back( dsim_##type##_array *array, type value ) \
    { _dsim_array_push_back( (_dsim_array*)array, &value, 1, sizeof(type) ); } \
    inline static void dsim_##type##_array_push_back_n( dsim_##type##_array *array, const type * data, uint32_t count ) \
    { _dsim_array_push_back( (_dsim_array*)array, data, count, sizeof(type) ); } \
    inline static void dsim_##type##_array_pop_back( dsim_##type##_array *array ) \
    { assert(array->count); --array->count; } \
    inline static void dsim_##type##_array_pop_back_n( dsim_##type##_array *array, uint32_t count ) \
    { assert(array->count >= count ); array->count -= count; } \
    inline static void dsim_##type##_array_clear( dsim_##type##_array *array ) \
    { array->count = 0; } \
    inline static void dsim_##type##_array_reset( dsim_##type##_array *array ) \
