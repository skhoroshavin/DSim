...
static int dsim_hash_is_full( const struct dsim_hash *h )
{
    return h->keys.count >= h->_hash.count * 11 / 16;
}

static struct dsim_hash_find_result dsim_hash_find( const struct dsim_hash *h, uint64_t key )
{
    struct dsim_hash_find_result res = _dsim_create_hash_set_find_result();
    if( !h->_hash.count )
        return res;
...
static uint32_t dsim_hash_append( struct dsim_hash *h, uint64_t key )
{
...
static uint32_t dsim_hash_make( struct dsim_hash *h, uint64_t key )
{
    const struct dsim_hash_find_result fr = dsim_hash_find( h, key );
    const uint32_t i = dsim_hash_append( h, key );
...
uint32_t dsim_hash_insert_multi( struct dsim_hash *h, uint64_t key );

static void dsim_hash_rehash( struct dsim_hash *h, uint32_t count )
{
    if( count <= h->_hash.count )
        return;

    struct dsim_hash new_set = dsim_hash_static_init(h->_hash.alloc);
...
int dsim_hash_contains( const struct dsim_hash *h, uint64_t key )
{
    return dsim_hash_find( h, key ).data_index != DSIM_INVALID_INDEX;
}
...
void dsim_hash_reserve( struct dsim_hash *h, uint32_t count )
{
    ...
    //dsim_hash_rehash( h, dsim_next_pow_2(count*16/11) );
}

uint32_t dsim_hash_insert( struct dsim_hash *h, uint64_t key )
{
    if( !h->_hash.count )
        dsim_hash_rehash( h, DSIM_HASH_INITIAL_HASH_SIZE );

    /*
    const uint32_t i = hash_internal::find_or_make(h, key);
    */

    if( dsim_hash_is_full(h) )
        dsim_hash_rehash( h, 2*h->_hash.count );
}
...
uint32_t dsim_hash_insert_multi( struct dsim_hash *h, uint64_t key )
{
    ...
    const uint32_t i = dsim_hash_make( h, key );
    if( dsim_hash_is_full(h) )
        dsim_hash_rehash( h, 2*h->_hash.count );
