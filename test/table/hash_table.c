...
{
    cr_assert( dsim_table_block_size( &table.table, 0 ) == count );
...
Test(hash_table_empty, assert_empty)
{
    ...
    cr_assert( dsim_table_find( &table.table, 25 ) == DSIM_INVALID_INDEX );
...
void init_hash_table_filled()
{
    ...
    memcpy( dsim_table_data( &table.table, 0, 1 ), test_data_1, sizeof(test_data_1) );
    memcpy( dsim_table_data( &table.table, 0, 2 ), test_data_2, sizeof(test_data_2) );
}
...
{
    test_hash_table_scheme();
    test_hash_table_count( test_count );

    cr_assert( memcmp( dsim_table_data( &table.table, 0, 0 ), test_data_0, sizeof(test_data_0) ) == 0 );
    cr_assert( memcmp( dsim_table_data( &table.table, 0, 1 ), test_data_1, sizeof(test_data_1) ) == 0 );
    cr_assert( memcmp( dsim_table_data( &table.table, 0, 2 ), test_data_2, sizeof(test_data_2) ) == 0 );
