...
static void dsim_process_number( struct flatcc_builder * b, dsim_type_table_t type )
{
    dsim_numeric_type_table_t ntype = (dsim_numeric_type_table_t)dsim_type_data( type );

    // LCOV_EXCL_START
    if( !dsim_type_size_is_present(type) )
        dsim_fatal( "Numeric type %s must have size!", dsim_type_name(type) );
    // LCOV_EXCL_END
...
static void dsim_process_struct( struct flatcc_builder *b, dsim_type_table_t type, dsim_type_vec_t prev_types )
{
    ...
    for( size_t i = 0; i != dsim_struct_field_vec_len(fields); ++i )
    {
        ...
        if( prev_types )
        {
            dsim_type_table_t type = dsim_types_scan_by_name( prev_types, dsim_struct_field_type(field) );
...
static void dsim_process_enum( struct flatcc_builder *b, dsim_type_table_t type )
{
...
static void dsim_process_reference( struct flatcc_builder *b, dsim_type_table_t type )
{
...
static void dsim_process_type( struct flatcc_builder *b, dsim_type_table_t type, dsim_type_vec_t prev_types )
{
...
static void dsim_process_types( struct flatcc_builder *b, dsim_type_vec_t types, dsim_type_vec_t prev_types )
{
...
static void dsim_process_layouts( struct flatcc_builder *b, dsim_layout_vec_t layouts )
{
    dsim_ddl_layouts_start( b );
    for( size_t i = 0; i != dsim_layout_vec_len(layouts); ++i )
    {
        dsim_layout_table_t layout = dsim_layout_vec_at(layouts, i);

        dsim_ddl_layouts_push_start( b );
        dsim_layout_name_clone( b, dsim_layout_name(layout) );

        dsim_layout_columns_start( b );
        ...
        for( size_t j = 0; j != dsim_column_vec_len(columns); ++j )
        {
            ...
            dsim_layout_columns_push_start( b );
            dsim_column_name_clone( b, dsim_column_name(column) );
            dsim_column_type_clone( b, dsim_column_type(column) );
            dsim_layout_columns_push_end( b );
        }
        dsim_layout_columns_end( b );

        dsim_ddl_layouts_push_end( b );
    }
    dsim_ddl_layouts_end( b );
}

static void *dsim_process_ddl( const void *raw_data, const void *prev_data, size_t *size_out )
{
    dsim_type_vec_t prev_types = 0;
    if( prev_data )
    {
        dsim_ddl_table_t prev_ddl = dsim_ddl_as_root( prev_data );
        prev_types = dsim_ddl_types( prev_ddl );
    }
    ...
    dsim_ddl_table_t ddl = dsim_ddl_as_root( raw_data );
    dsim_ddl_name_clone( &b, dsim_ddl_name(ddl) );

    dsim_process_types( &b, dsim_ddl_types(ddl), prev_types );
    dsim_process_layouts( &b, dsim_ddl_layouts(ddl) );
